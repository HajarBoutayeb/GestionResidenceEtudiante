        ...
        <div class="container">
            ...
            <%
                ...
                if (idParam == null || idParam.isEmpty()) {
                    out.println("<p class='error-message'>Aucun ID de technicien fourni.</p>");
                } else {
                    int id = Integer.parseInt(idParam);
                    String nom = "", email = "", motDePasse = "", telephone = "", specialite = "";
                    ...
                    try {
                        ...
                        // Requête SQL pour récupérer les informations du technicien
                        String sql = "SELECT * FROM techniciens WHERE id = ?";
                        PreparedStatement pstmt = conn.prepareStatement(sql);
                        pstmt.setInt(1, id);
                        ResultSet rs = pstmt.executeQuery();

                        // Si un technicien est trouvé, récupérer ses informations
                        if (rs.next()) {
                        ...
                        e.printStackTrace();
                        out.println("<p class='error-message'>Erreur lors de la récupération des données du technicien.</p>");
                    }
            %>
            <form action="ModifierTechnicienServlet" method="post">
                ...
                <input type="password" id="mot_de_passe" name="mot_de_passe" value="<%= motDePasse %>" required><br>

                <label for="telephone">Téléphone:</label>
                <input type="text" id="telephone" name="telephone" value="<%= telephone %>" required><br>

                <label for="specialite">Spécialité:</label>
                <select id="specialite" name="specialite" required>
                    <option value="plomberie" <% if (specialite.equals("plomberie")) out.print("selected"); %>>Plomberie</option>
                    <option value="électricité" <% if (specialite.equals("électricité")) out.print("selected"); %>>Électricité</option>
                    <option value="maintenance générale" <% if (specialite.equals("maintenance générale")) out.print("selected"); %>>Maintenance Générale</option>
                    <option value="autre" <% if (specialite.equals("autre")) out.print("selected"); %>>Autre</option>
                </select><br>
